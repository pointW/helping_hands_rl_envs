...
class TestBulletBowlStacking(unittest.TestCase):
  workspace = np.asarray([[0.25, 0.65],
                          [-0.2, 0.2],
                          [0.01, 0.25]])
  env_config = {'workspace': workspace, 'max_steps': 100, 'obs_size': 32, 'render': False, 'fast_mode': True,
                ...
                'view_type': 'camera_center_xyz', 'obs_type': 'pixel'}
  ...
  def testPlanner2(self):
    ...
    num_processes = 1
    env = env_factory.createEnvs(num_processes,  'close_loop_block_pulling', self.env_config, self.planner_config)
    total = 0
    s = 0
    step_times = []
    env.reset()
